      ...
      R.id.applyButton->{ //적용 버튼 누를때
        ...
        if (UserInfo.rankingLatLng != null) {
          ...
          if (requireView().tuneRadioBtnExecute.isChecked) {
            ...
            FBRanking().getFilterRange(requireView(), UserInfo.rankingLatLng!!, tuneDistance, "execute")
          } else {
            requireView().rankingfiltermode.text = "좋아요"
            FBRanking().getFilterRange(requireView(), UserInfo.rankingLatLng!!, tuneDistance, "likes")
          }
          disappearAnimation()
        }
      }
